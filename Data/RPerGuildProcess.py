...
df = pd.read_csv('roleplayerguild--all_char-ic-ooc_part1.csv')

def remove_html_tags(row):
    """Removes HTML tags from a string"""
    pattern = re.compile(r'<.*?>')
    return re.sub(pattern, '', row['message'])
df['message'] = df.apply(remove_html_tags, axis=1)

"""def remove_errant_slash(row):
    Removes errant slashes from a string
    return row['message'].replace('/', '')
df['message'] = df.apply(remove_errant_slash, axis=1)"""

def remove_unicode_escape(row):
    """Removes unicode escape characters from a string"""
    pattern = re.compile(r'u\d+')
    return re.sub(pattern, '', row['message'])
df['message'] = df.apply(remove_unicode_escape, axis=1)
...
def get_thread_messages(thread_title):
    ...
    df[~df['thread_type'].str.contains('OOC')]


    # Split the concatenated messages into two lists
    list1 = [concatenated_messages.iloc[0], end_name]
    list2 = []
    num = 0
